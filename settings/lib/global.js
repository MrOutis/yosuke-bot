...
return `*Título:* ${data.resultado.titulo}\n*Data de Upload:* ${data.resultado.publicado}`
}
...
exports.playMusic2_Result = (bla, pushname) => {
return `Olá *${pushname}*, estou realizando o download, confira as informações encontradas:\n\n*Título:* ${bla.resultado.título}\n*Canal:* ${bla.resultado.canal}\n*Data de Publicação:* ${bla.resultado.publicado}`
}
...
return `*Irei enviar 5 resultados, são eles:*\n1° _${data.resultado[1].title}_\n2° _${data.resultado[2].title}_\n3° _${data.resultado[3].title}_\n4° _${data.resultado[4].title}_\n5° _${data.resultado[5].title}_`
}
...
return `📚 *Wikipédia:*\n\n${wikis.data.query.pages[Object.keys(wikis.data.query.pages)].extract}`
}
...
return `*O participante foi removido do grupo* - Motivo: Membro comum com mensagem de marcação de todos do grupo, por conta disso irei remover do grupo, qualquer coisa entre em contato com um administrador ou com a minha dona...`
}

exports.absenceRecordOwner = (NickDono, tabelin) => {
return `Olá, o(a) meu(a) proprietário(a) "${NickDono}" se encontra ausente no momento.\n↺Desde do Horário: ${tabelin.Ausente_Desde}\n\n☇ Mensagem de Ausência: ${tabelin.Motivo_Da_Ausência}`
}
...
return `Olá *${pushname}*, minha proprietária ativou o modo aluguel, ou seja, os comandos não estão liberados aqui. Somente com a autorização dela eu posso responder comandos dados pela administração ou participantes deste grupo / comunidade.`
}
...
return `Mensagem proibida detectada, porém  admin logo a punição será anulada.`
}
...
return `Olá sou um bot , no momento não tou aceitando pv para atualizações somente em grupo acesse: https://chat.whatsapp.com/JAfps2cR8FsG7wdI9aYjZ8`
}
